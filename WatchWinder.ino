...
enum Button {
  ...
  LEFT,
  SELECT,
  ENC // Para el pulsador del potenciómetro
};
...
const int RANGE_ENC_MIN = 750;     const int RANGE_ENC_MAX = 850;
const int RANGE_NONE_MIN = 900;    // Valor cuando no se pulsa ningún botón (aprox. 900-1023)

// Variables de anti-rebote
unsigned long lastDebounceTime = 0;
const long DEBOUNCE_DELAY = 200; // 200 ms es un buen valor para anti-rebote
...
void saveStateToEEPROM() {
  ...
  lcd.setCursor(0,1);
  lcd.print("Reinicia sistema"); // Mensaje para indicar que se debe reiniciar
...
void setup() {
  ...
  digitalWrite(RELE_PIN, luz ? LOW : HIGH); // Configura el relé según el estado cargado (asumiendo LOW = encendido para luz)
    ...
    switch (pressedButton) {
      case SELECT:        
        if (currentLcdMode == 5) { // Si estamos en modo configuración RPM
          // Salir del modo configuración y volver al modo de inicio o pausa
          currentLcdMode = isRunning ? 1 : 0; // Si estaba "running" antes de entrar al modo config, va a modo girando; si no, a inicio.
                                            // Nota: Si estaba pausado manualmente, al salir de configuración vuelve a "inicio" (modo 0), no a "pausado" (modo 4). Esto es una simplificación.
          if(wasRunningBeforePowerOff && currentLcdMode == 0) { // Si estaba corriendo antes del apagado, pero ahora está en 0, lo ponemos en 4
              currentLcdMode = 4;
          }
          // No se guarda la EEPROM aquí, se guarda solo con DOWN explícitamente en modo 5
        } else {
      ...
      case DOWN:
        if (currentLcdMode == 5) { // Si estamos en modo configuración RPM
            ...
            luz = !luz; // Alterna el estado de la luz (y relé)
            if (luz){
      ...
      case ENC: // Botón del potenciómetro
        if (!isRunning) { // Si el sistema NO está corriendo (inicio, pausa manual, o completado)
          currentLcdMode = 5; // Entrar en modo configuración RPM
          lastLcdDisplayedMode = -1; // Forzar redibujo
        }
        // Si el sistema está corriendo (isRunning == true), este botón no hará nada
        break;        
    }    
  }    
}
